...
- Browser-only mode: provider choice, model, and non-secret provider options are saved in the browser. API keys are saved in browser session storage and are not CLI environment variables.
...
When Bridge has AI configuration, chat requests go through Bridge and use the provider credentials saved there. Otherwise Pondview sends AI requests from the browser with the settings saved in **AI Provider Configuration**.
