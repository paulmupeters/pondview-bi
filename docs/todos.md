...
- multiple columns in dashboard? but not too much, also we could make the cards not the same size as the charts, now they become too large if on same row as chart, but would be nice if we then could stack two cards on the same row as 1 chart.
