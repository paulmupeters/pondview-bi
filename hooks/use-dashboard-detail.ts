...
export function useDashboardDetail(dashboardId: string): UseDashboardDetailReturn {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const foundDashboard = list.dashboards.find((item) => item.id === dashboardId) ?? null;
        if (!cancelled) {
  ...
  const handleChartDelete = useCallback(
    async (chartId: string) => {
      try {
        const res = await fetch(`/api/dashboard/${dashboardId}/charts?chartId=${chartId}`, {
          method: "DELETE",
        });
