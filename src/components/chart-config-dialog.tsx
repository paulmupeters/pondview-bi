  ...
  const handleButtonClick = () => {
    if (inline && containerRef.current) {
      ...
      const getValue = (name: string): string | null => {
        const el = container.querySelector(`[name="${name}"]`) as HTMLInputElement | HTMLSelectElement | null;
        return el?.value ?? null;
      };
      
      // Helper to get checked radio value
      const getRadioValue = (name: string): string | null => {
        const el = container.querySelector(`[name="${name}"]:checked`) as HTMLInputElement | null;
        return el?.value ?? null;
      };
      ...
      formData.append("showYAxis", getRadioValue("showYAxis") || (config?.showYAxis !== false ? "true" : "false"));
      formData.append("showDots", getRadioValue("showDots") || (config?.showDots !== false ? "true" : "false"));
      formData.append("showTooltip", getRadioValue("showTooltip") || (config?.showTooltip !== false ? "true" : "false"));
      formData.append("lineSize", getValue("lineSize") || String(config?.lineSize ?? 2));
      formData.append("suffixLabelY", getValue("suffixLabelY") || "");
      formData.append("labelYAngle", getValue("labelYAngle") || String(config?.labelYAngle ?? -90));
      formData.append("referenceLineLabel", getValue("referenceLineLabel") || "");
  ...
  const formContent = (
    <div ref={inline ? containerRef : undefined}>
      ...
      <TabsContent value="data" className="space-y-6 mt-4">
        ...
        <fieldset className="space-y-3">
          <legend className="text-sm font-medium">Y-Axis Columns (values)</legend>
      ...
      <TabsContent value="appearance" className="space-y-6 mt-4">
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-xs text-gray-500">Size of line unit in pixel</p>
          </div>
          <Input
            id="lineSize"
            name="lineSize"
      ...
      <TabsContent value="advanced" className="space-y-6 mt-4">
        ...
        <div className="space-y-3">
          <div>
            <label htmlFor="referenceLineLabel" className="text-sm font-medium">
              Reference line label
            </label>
            <p className="text-xs text-gray-500">Display reference line label</p>
          </div>
          <Input
