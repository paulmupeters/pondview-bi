...
export default function ViewDataPage() {
  ...
  const getEntryKey = (table: any) => {
    return `${table.type}-${table.databasePath}-${table.schema ?? table.table ?? "unknown"}`;
  };

  const hasMultipleTables = (table: any) => {
    return table.tables && Array.isArray(table.tables) && table.tables.length > 0;
  };
    ...
    <div className="mx-auto flex h-full w-full max-w-5xl flex-col gap-8 overflow-y-auto px-6 py-10">
      <header className="space-y-3">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Review the schemas or tables you have connected locally. These entries are stored
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    {hasMultiple && (
                      <Button
                        ...
                        className="text-xs"
                      >
                        {isExpanded ? "Hide Tables" : `Show Tables (${table.tables?.length})`}
                      </Button>
                    )}
